...
enum class AwsAccessType {
  kUndefined,
  kDefault,     // Use AWS SDK's default credential chain
  kSimple,      // Use simple secret/access key creds
  kInstance,    // Use Provider credentials
  kTaskRole,
  kEnvironment, // Use credentials from environment
  kConfig,      // Use credentials from config file
  kAnonymous,   // Use anonymous credentials
};
...
struct CloudEnvOptions {
  ...
  std::shared_ptr<CloudLogController> log_controller;
...
public:
  static const std::string kCloudEnvOpts /*="CloudEnvOptions"*/;
  static const std::string kCloudEnvName /*="CloudEnv"*/;
 protected:
