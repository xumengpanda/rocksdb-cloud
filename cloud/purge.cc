...
Status CloudEnvImpl::FindObsoleteFiles(const std::string& bucket_name_prefix,
  ...
  std::unique_ptr<ManifestReader> extractor(
      new ManifestReader(GetInfoLogger(), this, bucket_name_prefix));
...
Status CloudEnvImpl::FindObsoleteDbid(
  ...
  if (st.ok()) {
    for (auto iter = dbid_list.begin(); iter != dbid_list.end(); ++iter) {
      ...
      st = provider_->ExistsCloudObject(GetDestBucketName(), path);
...
Status CloudEnvImpl::extractParents(const std::string& bucket_name_prefix,
  ...
  for (auto iter = dbid_list.begin(); iter != dbid_list.end(); ++iter) {
    ...
    st = provider_->GetCloudObject(bucket_name_prefix, cloudfile, localfile);
    ...
    if (!st.ok()) {
      Log(InfoLogLevel::ERROR_LEVEL, GetInfoLogger(), "[pg] Unable to read %s %s",
          localfile.c_str(), st.ToString().c_str());
      return st;
    }
    st = base_env_->DeleteFile(localfile);
    if (!st.ok()) {
      Log(InfoLogLevel::ERROR_LEVEL, GetInfoLogger(), "[pg] Unable to delete %s %s",
          localfile.c_str(), st.ToString().c_str());
      return st;
    }
