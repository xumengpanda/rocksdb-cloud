...
class KafkaController : public CloudLogControllerImpl {
 public:
  ...
  const char* Name() const override { return CloudOptionNames::kNameKafka; }
...
Status KafkaController::PrepareOptions(const ConfigOptions& opts) {
  ...
  if (cloud_env == nullptr) {
    status_ =
        Status::InvalidArgument("Kafka requires cloud env ", opts.env->GetId());
  } else {
    ...
    if (!producer_) {
      ...
      Log(InfoLogLevel::ERROR_LEVEL, cloud_env->info_log_,
          "[%s] Kafka producer error: %s", cloud_env->Name(),
          status_.ToString().c_str());
    } else if (!consumer_) {
      ...
      Log(InfoLogLevel::ERROR_LEVEL, cloud_env->info_log_,
          "[%s] Kafka consumer error: %s", cloud_env->Name(),
          status_.ToString().c_str());
    } else {
