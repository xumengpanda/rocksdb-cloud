...
Status AwsEnv::NewSequentialFileCloud(const std::string& bucket_prefix,
  ...
  Status st = NewS3ReadableFile(this, bucket_prefix, fname, &file);
  if (!st.ok()) {
    return st;
  }
    ...
    if (cloud_env_options.keep_local_sst_files) {
      ...
      if (st.ok() && cloud_env_options.validate_filesize) {
        ...
        if (!stax.ok() || remote_size != local_size) {
          std::string msg = "[aws] HeadObject src " + fname +
...
Status AwsEnv::HeadObject(const std::string& bucket_prefix,
  ...
  if (!isSuccess) {
    ...
    if (s3err == Aws::S3::S3Errors::NO_SUCH_BUCKET ||
        s3err == Aws::S3::S3Errors::NO_SUCH_KEY ||
        s3err == Aws::S3::S3Errors::RESOURCE_NOT_FOUND) {
      return Status::NotFound(path, error.getMessage());
    }
    return Status::IOError(path, error.getMessage());
  }
  if (metadata != nullptr) {
  ...
  if (modtime != nullptr) {
      *modtime = res.GetLastModTime().Millis();
  }
  return Status::OK();
}
...
Status AwsEnv::GetPathForDbid(const std::string& bucket_prefix,
  ...
  const std::string kDirnameTag = "dirname";
