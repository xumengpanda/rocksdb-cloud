...
Status S3StorageProvider::Initialize(CloudEnv* env) {
  ...
  if (!env->SrcMatchesDest() && env->HasSrcBucket() &&
      env->HasDestBucket()) {
...
Status S3StorageProvider::CreateBucket(const std::string& bucket) {
  ...
  Aws::S3::Model::BucketLocationConstraint bucket_location =
    ...
    GetBucketLocationConstraintForName(ToAwsString(env_->GetCloudEnvOptions().dest_bucket.GetRegion()));
...
Status S3StorageProvider::EmptyBucket(const std::string& bucket_name,
  ...
  Log(InfoLogLevel::DEBUG_LEVEL, env_->GetLogger(),
      "[s3] EmptyBucket going to delete %" ROCKSDB_PRIszt " objects in bucket %s",
...
Status S3StorageProvider::DoNewCloudReadableFile(
  ...
  result->reset(
      new S3ReadableFile(s3client_, env_->GetLogger(), bucket, fname, fsize));
  return Status::OK();
}
...
Status S3StorageProvider::DoPutObject(const std::string& local_file,
  ...
  if (s3client_->HasTransferManager()) {
    ...
    if (handle->GetStatus() != Aws::Transfer::TransferStatus::COMPLETED) {
      ...
      std::string errmsg(error.GetMessage().c_str(), error.GetMessage().size());
      Log(InfoLogLevel::ERROR_LEVEL, env_->GetLogger(),
          "[s3] UploadFile %s/%s, size %" PRIu64 ", ERROR %s", bucket_name.c_str(),
    ...
    if (!outcome.IsSuccess()) {
      ...
      std::string errmsg(error.GetMessage().c_str(), error.GetMessage().size());
      Log(InfoLogLevel::ERROR_LEVEL, env_->GetLogger(),
          "[s3] PutObject %s/%s, size %" PRIu64 ", ERROR %s", bucket_name.c_str(),
          object_path.c_str(), file_size, errmsg.c_str());
      return Status::IOError(local_file, errmsg);
    }
  }
...
namespace rocksdb {
Status CloudStorageProvider::CreateS3Provider(std::unique_ptr<CloudStorageProvider>* provider) {
#ifndef USE_AWS
  provider->reset();
  return Status::NotSupported("In order to use S3, make sure you're compiling with USE_AWS=1");
#else
