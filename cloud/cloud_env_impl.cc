...
CloudEnvImpl::CloudEnvImpl(Env* base, const CloudEnvOptions& opts,
                           std::unique_ptr<CloudStorageProvider> provider,
                           std::unique_ptr<CloudLogController> controller)
  : CloudEnv(base, opts, std::move(provider), std::move(controller)), purger_is_running_(true) {
  scheduler_ = CloudScheduler::Get();
}

CloudEnvImpl::~CloudEnvImpl() {
  printf("MJR: %p Deleting CloudEnvImpl controller=%p provider=%p\n",
         this, controller_.get(), provider_.get());
  if (controller_) {
    controller_->StopTailingStream();
  }
  {
...
Status CloudEnvImpl::ExistsCloudObject(const std::string& fname) {
  ...
  if (HasDestBucket()) {
    st = provider_->ExistsCloudObject(GetDestBucketName(), destname(fname));
  }
  if (st.IsNotFound() && HasSrcBucket() && !SrcMatchesDest()) {
    st = provider_->ExistsCloudObject(GetSrcBucketName(), srcname(fname));
  }
  return st;
}
...
Status CloudEnvImpl::GetCloudObject(const std::string& fname) {
  Status st = Status::NotFound();
  if (HasDestBucket()) {
    st = provider_->GetCloudObject(GetDestBucketName(), destname(fname), fname);
  }
  if (st.IsNotFound() && HasSrcBucket() && !SrcMatchesDest()) {
    st = provider_->GetCloudObject(GetSrcBucketName(), srcname(fname), fname);
  }
  return st;
}
...
Status CloudEnvImpl::GetCloudObjectSize(const std::string& fname,
  ...
  Status st = Status::NotFound();
  if (HasDestBucket()) {
    st = provider_->GetCloudObjectSize(GetDestBucketName(), destname(fname), remote_size);
  }
  if (st.IsNotFound() && HasSrcBucket() && !SrcMatchesDest()) {
    st = provider_->GetCloudObjectSize(GetSrcBucketName(), srcname(fname), remote_size);
  }
  return st;
}
...
Status CloudEnvImpl::GetCloudObjectModificationTime(const std::string& fname,
  ...
  Status st = Status::NotFound();
  if (HasDestBucket()) {
    st = provider_->GetCloudObjectModificationTime(GetDestBucketName(), destname(fname), time);
  }
  if (st.IsNotFound() && HasSrcBucket() && !SrcMatchesDest()) {
    st = provider_->GetCloudObjectModificationTime(GetSrcBucketName(), srcname(fname), time);
  }
  return st;
}
  ...
  if (HasSrcBucket()) {
    ...
    if (!st.ok()) {
      Log(InfoLogLevel::ERROR_LEVEL, GetInfoLogger(),
          ...
          provider_->Name(), st.ToString().c_str());
      return st;
    }
  }
  if (HasDestBucket() && !SrcMatchesDest()) {
    st = provider_->ListCloudObjects(GetDestBucketName(), GetDestObjectPath(), result);
    if (!st.ok()) {
      Log(InfoLogLevel::ERROR_LEVEL, GetInfoLogger(),
...
Status CloudEnvImpl::NewCloudReadableFile(
  ...
  if (HasDestBucket()) {  // read from destination
    st = provider_->NewCloudReadableFile(
...
Status CloudEnvImpl::NewSequentialFile(const std::string& logical_fname,
  ...
  if (sstfile || manifest || identity) {
    ...
    return controller_->NewSequentialFile(fname, result, options);
  }
...
Status CloudEnvImpl::NewSequentialFileCloud(
  ...
  std::unique_ptr<CloudStorageReadableFile> file;
  Status st = provider_->NewCloudReadableFile(
...
Status CloudEnvImpl::NewRandomAccessFile(
  ...
  if (sstfile || manifest || identity) {
    ...
    if (cloud_env_options.keep_local_sst_files || !sstfile) {
      ...
      if (st.ok() && sstfile && cloud_env_options.validate_filesize) {
        ...
        if (stax.IsNotFound() && !HasDestBucket()) {
          ...
          Log(InfoLogLevel::ERROR_LEVEL, GetInfoLogger(), "%s", msg.c_str());
    ...
    st = controller_->NewRandomAccessFile(
...
Status CloudEnvImpl::NewWritableFile(const std::string& logical_fname,
  ...
  if (HasDestBucket() && (sstfile || identity || manifest)) {
    ...
    provider_->NewCloudWritableFile(
    ...
    std::unique_ptr<CloudLogWritableFile> f(
        controller_->CreateWritableFile(fname, options));
    if (!f || !f->status().ok()) {
      std::string msg = std::string("[") + Name() + "] NewWritableFile";
      s = Status::IOError(msg, fname.c_str());
      Log(InfoLogLevel::ERROR_LEVEL, GetInfoLogger(), "%s src %s %s", msg.c_str(),
  ...
  Log(InfoLogLevel::DEBUG_LEVEL, GetInfoLogger(), "[%s] NewWritableFile src %s %s",
...
Status CloudEnvImpl::FileExists(const std::string& logical_fname) {
  ...
  if (sstfile || manifest || identity) {
    ...
    st = controller_->FileExists(fname);
  } else {
    st = base_env_->FileExists(fname);
  }
  Log(InfoLogLevel::DEBUG_LEVEL, GetInfoLogger(), "[%s] FileExists path '%s' %s",
...
Status CloudEnvImpl::GetChildren(const std::string& path,
                                 std::vector<std::string>* result) {
  Log(InfoLogLevel::DEBUG_LEVEL, GetInfoLogger(), "[%s] GetChildren path '%s' ",
      Name(), path.c_str());
  result->clear();
...
Status CloudEnvImpl::GetFileSize(const std::string& logical_fname,
  ...
  *size = 0L;
  ...
  if (sstfile) {
    ...
    st = controller_->GetFileSize(fname, size);
  } else {
    st = base_env_->GetFileSize(fname, size);
  }
  Log(InfoLogLevel::DEBUG_LEVEL, GetInfoLogger(),
...
Status CloudEnvImpl::GetFileModificationTime(const std::string& logical_fname,
  ...
  *time = 0;
  ...
  if (sstfile) {
    ...
    st = controller_->GetFileModificationTime(fname, time);
  } else {
    st = base_env_->GetFileModificationTime(fname, time);
  }
  Log(InfoLogLevel::DEBUG_LEVEL, GetInfoLogger(),
...
Status CloudEnvImpl::NewDirectory(const std::string& name,
  ...
  Log(InfoLogLevel::DEBUG_LEVEL, GetInfoLogger(), "[%s] NewDirectory name '%s'",
      Name(), name.c_str());
  ...
  Log(InfoLogLevel::DEBUG_LEVEL, GetInfoLogger(), "[%s] NewDirectory name %s ok",
      Name(), name.c_str());
  return Status::OK();
}
...
Status CloudEnvImpl::CreateDir(const std::string& dirname) {
  Log(InfoLogLevel::DEBUG_LEVEL, GetInfoLogger(), "[%s] CreateDir dir '%s'", Name(),
      dirname.c_str());
  Status st;
  ...
  Log(InfoLogLevel::DEBUG_LEVEL, GetInfoLogger(), "[%s] CreateDir dir %s %s", Name(),
...
Status CloudEnvImpl::CreateDirIfMissing(const std::string& dirname) {
  Log(InfoLogLevel::DEBUG_LEVEL, GetInfoLogger(), "[%s] CreateDirIfMissing dir '%s'",
      Name(), dirname.c_str());
  Status st;
...
Status CloudEnvImpl::DeleteDir(const std::string& dirname) {
  Log(InfoLogLevel::DEBUG_LEVEL, GetInfoLogger(), "[%s] DeleteDir src '%s'", Name(),
      dirname.c_str());
  Status st = base_env_->DeleteDir(dirname);
  Log(InfoLogLevel::DEBUG_LEVEL, GetInfoLogger(), "[%s] DeleteDir dir %s %s", Name(),
      dirname.c_str(), st.ToString().c_str());
  return st;
};
...
Status CloudEnvImpl::DeleteFile(const std::string& logical_fname) {
  ...
  if (sstfile || manifest || identity) {
    ...
    st = controller_->status();
    if (st.ok()) {
      // Log a Delete record to controller stream
      std::unique_ptr<CloudLogWritableFile> f(
              controller_->CreateWritableFile(fname, EnvOptions()));
      if (!f || !f->status().ok()) {
        std::string msg =
            "[" + std::string(controller_->Name()) +
            "] DeleteFile";
  ...
  Log(InfoLogLevel::DEBUG_LEVEL, GetInfoLogger(), "[%s] DeleteFile file %s %s",
      Name(), fname.c_str(), st.ToString().c_str());
  return st;
}
...
Status CloudEnvImpl::CopyLocalFileToDest(const std::string& local_name,
  ...
  return provider_->PutCloudObject(local_name, GetDestBucketName(), dest_name);
}
...
Status CloudEnvImpl::DeleteCloudFileFromDest(const std::string& fname) {
  ...
  auto doDeleteFile = [this, base](void*) {
    ...
    auto st = provider_->DeleteCloudObject(
...
Status CloudEnvImpl::SaveIdentityToCloud(const std::string& localfile,
  ...
  if (st.ok()) {
    st = provider_->PutCloudObject(localfile, GetDestBucketName(), idfile);
  }
  ...
  if (src_object_path.empty() && dest_object_path.empty()) {
    ...
    if (load_status.ok()) {
      ...
      if (!local_manifest_exists.ok()) {
        Log(InfoLogLevel::WARN_LEVEL, GetInfoLogger(),
...
Status CloudEnvImpl::GetCloudDbid(const std::string& local_dir,
  ...
  if (HasSrcBucket()) {
    Status st = provider_->GetCloudObject(
...
Status CloudEnvImpl::Prepare() {
  auto info_log = GetInfoLogger();
  Header(info_log, "     %s.src_bucket_name: %s", Name(),
         cloud_env_options.src_bucket.GetBucketName().c_str());
  Header(info_log, "     %s.src_object_path: %s", Name(),
         cloud_env_options.src_bucket.GetObjectPath().c_str());
  Header(info_log, "     %s.src_bucket_region: %s", Name(),
         cloud_env_options.src_bucket.GetRegion().c_str());
  Header(info_log, "     %s.dest_bucket_name: %s", Name(),
         cloud_env_options.dest_bucket.GetBucketName().c_str());
  Header(info_log, "     %s.dest_object_path: %s", Name(),
         cloud_env_options.dest_bucket.GetObjectPath().c_str());
  Header(info_log, "     %s.dest_bucket_region: %s", Name(),
  ...
  if (cloud_env_options.src_bucket.GetBucketName().empty() !=
    ...
    if (!provider_) {
      ...
      Header(info_log, "     %s.storage_provider: %s", Name(),
             provider_->Name());
      s = provider_->Prepare(this);
    }
    if (s.ok()) {
      if (controller_) {
        Header(info_log, "     %s.log controller: %s", Name(),
               controller_->Name());
        s = controller_->Prepare(this);
...
void CloudEnvImpl::TEST_Initialize() {
  if (provider_) {
    auto provider = CloudStorageProviderImpl::AsImpl(provider_.get());
    if (provider != nullptr) {
      provider->TEST_Initialize();
    }
  }
  if (controller_) {
    auto controller = CloudLogControllerImpl::AsImpl(controller_.get());
