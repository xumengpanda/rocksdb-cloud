...
Status CloudStorageReadableFile::Read(uint64_t offset, size_t n, Slice* result,
                                      char* scratch) const {
  Log(InfoLogLevel::DEBUG_LEVEL, info_log_,
      "[%s] CloudReadableFile reading %s at offset %" PRIu64 " size %" ROCKSDB_PRIszt, 
  ...
  *result = Slice();
  ...
  if (st.ok()) {
    *result = Slice(scratch, bytes_read);
    Log(InfoLogLevel::DEBUG_LEVEL, info_log_,
        "[%s] CloudReadableFile file %s filesize %" PRIu64 " read %" PRIu64 " bytes", 
...
Status CloudStorageReadableFile::Skip(uint64_t n) {
  Log(InfoLogLevel::DEBUG_LEVEL, info_log_,
      "[%s] CloudReadableFile file %s skip %" PRIu64, Name(), fname_.c_str(), n);
...
Status CloudStorageProviderImpl::Verify() const {
  if (!status_.ok()) {
    ...
    return Status::InvalidArgument("Storage Provider not initialized: ", Name());
  } else {
    return Status::OK();
  }
}
...
Status CloudStorageProviderImpl::GetObject(const std::string& bucket_name,
  ...
  if (local_size != remote_size) {
    ...
    Log(InfoLogLevel::ERROR_LEVEL, env_->GetLogger(),
        "[%s] GetObject %s/%s local size %" PRIu64 " != cloud size "
        "%" PRIu64 ". %s", Name(),
