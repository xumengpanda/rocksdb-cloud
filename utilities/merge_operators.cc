...
std::shared_ptr<MergeOperator> MergeOperators::CreateFromStringId(
  ...
  std::shared_ptr<MergeOperator> op;
  ConfigOptions opts;
  Status s = MergeOperator::CreateFromString(id, opts, &op);
  if (s.ok()) {
    return op;
  } else {
    return nullptr;
...
Status MergeOperator::CreateFromString(const std::string& value,
                                       const ConfigOptions& opts,
                                       std::shared_ptr<MergeOperator>* result) {
  Status status =
      LoadSharedObject<MergeOperator>(value, CreateMergeOperator, opts, result);
  return status;
}
