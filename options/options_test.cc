...
TEST_F(OptionsTest, GetOptionsFromMapTest) {
  ...
  ASSERT_NOK(GetColumnFamilyOptionsFromMap(base_cf_opt, cf_options_map,
                                           cfg_opts, &new_cf_opt));
  ASSERT_OK(
  ...
  ConfigOptions ignore = cfg_opts;
  ignore.ignore_unknown_options = true;

  ASSERT_OK(GetColumnFamilyOptionsFromMap(base_cf_opt, cf_options_map, ignore,
                                          &new_cf_opt));
  cfg_opts.sanity_level =
      OptionsSanityCheckLevel::kSanityLevelLooselyCompatible;
  ASSERT_OK(
      RocksDBOptionsParser::VerifyCFOptions(base_cf_opt, new_cf_opt, cfg_opts));

  cfg_opts.sanity_level = OptionsSanityCheckLevel::kSanityLevelExactMatch;
  ASSERT_NOK(
      RocksDBOptionsParser::VerifyCFOptions(base_cf_opt, new_cf_opt, cfg_opts));
...
TEST_F(OptionsTest, GetColumnFamilyOptionsFromStringTest) {
  ...
  ASSERT_OK(
      GetColumnFamilyOptionsFromString(base_cf_opt, "", cfg_opts, &new_cf_opt));
  ...
  ASSERT_OK(GetColumnFamilyOptionsFromString(
      base_cf_opt, "write_buffer_size=6;", cfg_opts, &new_cf_opt));
  ASSERT_EQ(new_cf_opt.write_buffer_size, 6U);
  ASSERT_OK(GetColumnFamilyOptionsFromString(
      base_cf_opt, "  write_buffer_size =  7  ", cfg_opts, &new_cf_opt));
  ASSERT_EQ(new_cf_opt.write_buffer_size, 7U);
  ASSERT_OK(GetColumnFamilyOptionsFromString(
      base_cf_opt, "  write_buffer_size =  8 ; ", cfg_opts, &new_cf_opt));
  ASSERT_EQ(new_cf_opt.write_buffer_size, 8U);
  ASSERT_OK(GetColumnFamilyOptionsFromString(
      base_cf_opt, "write_buffer_size=9;max_write_buffer_number=10", cfg_opts,
      &new_cf_opt));
  ...
  ASSERT_OK(GetColumnFamilyOptionsFromString(
      base_cf_opt, "comparator=" + kCompName + ";", cfg_opts, &new_cf_opt));
  ...
  ASSERT_OK(GetColumnFamilyOptionsFromString(
      base_cf_opt, "merge_operator=" + kMoName + ";", cfg_opts, &new_cf_opt));
  ASSERT_EQ(kMoName, std::string(new_cf_opt.merge_operator->Name()));

  // Wrong key/value pair
  ASSERT_NOK(GetColumnFamilyOptionsFromString(
      base_cf_opt, "write_buffer_size=13;max_write_buffer_number;", cfg_opts,
      &new_cf_opt));
  ASSERT_OK(
      RocksDBOptionsParser::VerifyCFOptions(base_cf_opt, new_cf_opt, cfg_opts));
  ...
  // Missing option name
  ASSERT_NOK(GetColumnFamilyOptionsFromString(
      base_cf_opt, "write_buffer_size=13; =100;", cfg_opts, &new_cf_opt));
  ASSERT_OK(
      RocksDBOptionsParser::VerifyCFOptions(base_cf_opt, new_cf_opt, cfg_opts));
  ...
  // Units (k)
  ASSERT_OK(GetColumnFamilyOptionsFromString(
      base_cf_opt, "max_write_buffer_number=15K", cfg_opts, &new_cf_opt));
  ...
  // Units (t)
  ASSERT_OK(GetColumnFamilyOptionsFromString(
      base_cf_opt, "write_buffer_size=20t;arena_block_size=21T", cfg_opts,
      &new_cf_opt));
  ...
  ASSERT_OK(GetColumnFamilyOptionsFromString(
      base_cf_opt, "optimize_filters_for_hits=false", cfg_opts, &new_cf_opt));

  ASSERT_NOK(GetColumnFamilyOptionsFromString(
      base_cf_opt, "optimize_filters_for_hits=junk", cfg_opts, &new_cf_opt));
  ASSERT_OK(
      RocksDBOptionsParser::VerifyCFOptions(base_cf_opt, new_cf_opt, cfg_opts));
...
TEST_F(OptionsTest, GetBlockBasedTableOptionsFromString) {
  ...
  ASSERT_NOK(factory->ConfigureFromString(
      "cache_index_and_filter_blocks=1;index_type=kBinarySearchXX", cfg_opts));
  ASSERT_EQ(table_opt.cache_index_and_filter_blocks,
            new_opt->cache_index_and_filter_blocks);
  ASSERT_EQ(table_opt.index_type, new_opt->index_type);
  ...
  ASSERT_NOK(factory->ConfigureFromString(
      "cache_index_and_filter_blocks=1;checksum=kxxHashXX", cfg_opts));
  ASSERT_EQ(table_opt.cache_index_and_filter_blocks,
            new_opt->cache_index_and_filter_blocks);
  ASSERT_EQ(table_opt.index_type, new_opt->index_type);
  ...
  ASSERT_NOK(
      factory->ConfigureFromString("cache_index_and_filter_blocks=1;"
  ...
  ASSERT_OK(
      factory->ConfigureFromString("block_cache={capacity=2M};"
...
TEST_F(OptionsTest, ColumnFamilyOptionsSerialization) {
  ...
  std::string base_options_file_content;
  ASSERT_OK(GetStringFromColumnFamilyOptions(base_opt, cfg_opts,
                                             &base_options_file_content));

  // Phase 3: Set new_opt from the derived string and expect
  //          new_opt == base_opt
  ASSERT_OK(GetColumnFamilyOptionsFromString(
      ColumnFamilyOptions(), base_options_file_content, cfg_opts, &new_opt));
