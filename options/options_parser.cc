...
Status PersistRocksDBOptions(const DBOptions& db_opt,
  ...
  s = GetStringFromDBOptions(db_opt, cfg_opts, &options_file_content);
  if (!s.ok()) {
    writable->Close();
    return s;
  }
  ...
  for (size_t i = 0; i < cf_opts.size(); ++i) {
    ...
    s = GetStringFromColumnFamilyOptions(cf_opts[i], cfg_opts,
    ...
    if (tf != nullptr) {
      ...
      s = tf->GetOptionString(cfg_opts, &options_file_content);
      if (!s.ok()) {
        return s;
      }
      writable->Append(options_file_content + "\n");
...
Status RocksDBOptionsParser::Parse(const std::string& file_name, FileSystem* fs,
  ...
  ConfigOptions cfg_opts(db_opt_);
  ...
  for (int line_num = 1;
    ...
    if (IsSection(line)) {
      s = EndSection(section, title, argument, opt_map, cfg_opts);
      opt_map.clear();
      if (!s.ok()) {
        return s;
      }
      ...
      if (cfg_opts.ignore_unknown_options && section == kOptionSectionVersion) {
...
Status RocksDBOptionsParser::EndSection(
  ...
  Status s;
  if (section == kOptionSectionDBOptions) {
    s = GetDBOptionsFromMap(DBOptions(), opt_map, &db_opt_,
                            cfg_opts.input_strings_escaped,
                            cfg_opts.ignore_unknown_options);
    if (!s.ok()) {
      return s;
    }
    db_opt_map_ = opt_map;
    ...
    s = GetColumnFamilyOptionsFromMap(ColumnFamilyOptions(), opt_map, cfg_opts,
                                      &cf_opts_.back());
    if (!s.ok()) {
      return s;
    }
...
Status RocksDBOptionsParser::VerifyTableFactory(const TableFactory* base_tf,
  ...
  if (base_tf && file_tf) {
    ...
    std::string mismatch;
    bool matches = base_tf->Matches(file_tf, options, &mismatch);
    if (!matches) {
      return Status::Corruption(
          "[RocksDBOptionsParser]: "
