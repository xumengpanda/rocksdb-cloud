...
static std::unordered_map<std::string, OptionTypeInfo> db_options_type_info = {
    ...
    {"persist_stats_to_disk",
     {offsetof(struct DBOptions, persist_stats_to_disk), OptionType::kBoolean,
      OptionVerificationType::kNormal, OptionTypeFlags::kNone,
      offsetof(struct ImmutableDBOptions, persist_stats_to_disk)}},
    {"stats_history_buffer_size",
     {offsetof(struct DBOptions, stats_history_buffer_size), OptionType::kSizeT,
      OptionVerificationType::kNormal, OptionTypeFlags::kMutable,
    ...
    {"write_dbid_to_manifest",
     {offsetof(struct DBOptions, write_dbid_to_manifest), OptionType::kBoolean,
      OptionVerificationType::kNormal, OptionTypeFlags::kNone, 0}},
    {"log_readahead_size",
    ...
    {"rate_limiter_bytes_per_sec",
     {offsetof(struct DBOptions, rate_limiter), OptionType::kUnknown,
      ...
      [](const std::string&, const std::string& value, const ConfigOptions&,
         char* addr) {
        if (!value.empty()) {
          auto* limiter = reinterpret_cast<std::shared_ptr<RateLimiter>*>(addr);
          limiter->reset(
...
Status GetMutableDBOptionsFromStrings(
  ...
  assert(new_options);
  ConfigurableStruct<MutableDBOptions> config(base_options,
                                              &db_options_type_info, true);
  ConfigOptions cfg_options;
  Status status = config.ConfigureFromMap(options_map, cfg_options);
  if (status.ok()) {
    *new_options = *(config.GetStructOptions());
  } else {
...
Status GetDBOptionsFromMapInternal(
  ...
  if (unsupported_options != nullptr) {
    status =
        config.ConfigureFromMap(opts_map, cfg_options, unsupported_options);
  } else {
    status = config.ConfigureFromMap(opts_map, cfg_options);
  }
  if (status.ok()) {
    *new_options = *(config.GetStructOptions());
    status = config.SanitizeOptions(*new_options);
  }
  if (!status.ok()) {
    *new_options = base_options;
  }
...
Status GetStringFromDBOptions(const DBOptions& db_options,
  ...
  ConfigurableStruct<DBOptions> config(db_options, &db_options_type_info,
                                       false);
...
Status GetDBOptionsFromMap(
    ...
    bool ignore_unknown_options) {
  return GetDBOptionsFromMapInternal(base_options, opts_map,
                                     input_strings_escaped, new_options,
                                     nullptr, ignore_unknown_options);
}
...
Status GetDBOptionNames(std::unordered_set<std::string>* opt_names,
  ...
  if (use_mutable) {
    MutableDBOptions opts;
    ConfigurableStruct<MutableDBOptions> config(opts, &db_options_type_info,
                                                true);
    return config.GetOptionNames(options, opt_names);
  } else {
    DBOptions opts;
    ConfigurableStruct<DBOptions> config(opts, &db_options_type_info, false);
    return config.GetOptionNames(options, opt_names);
  }
}
...
Status RocksDBOptionsParser::VerifyDBOptions(
  ...
  ConfigurableStruct<DBOptions> base_config(base_opt, &db_options_type_info,
                                            false);
  ConfigurableStruct<DBOptions> persisted_config(persisted_opt,
                                                 &db_options_type_info, false);
  std::string mismatch;
  if (!base_config.Matches(&persisted_config, cfg_opts, &mismatch)) {
    std::string base_value;
    std::string persisted_value;
    const size_t kBufferSize = 2048;
    char buffer[kBufferSize];

    if (base_config.GetOption(mismatch, cfg_opts, &base_value).ok() &&
        persisted_config.GetOption(mismatch, cfg_opts, &persisted_value).ok()) {
